...
public class BoardView extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        Log.d("BoardView", "x=" + x + "y=" + y);
        currX = x;
        currY = y;


        for(int i = (int)x-15;i < x+15; i++)
        {
            for(int j= (int)y-15; j < y +15; j++)
            {
                reper.add(new Point(i,j));

            }
        }
        reper = reper.getNextState();
        invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        for(Point point :reper.liveCells)
        {
            canvas.drawRect(point.x,point.y,point.x+1,point.y+1,mTextPaint);
        }
